...
const Modal = ({ image, name, title, intro }) => {
  ...
  return (
    ...
    <div className="relative flex flex-col lg:grid lg:grid-cols-2 lg:h-2/3 lg:w-2/3 lg:max-w-[1000px] lg:max-h-[550px] h-[75vh] w-[90%] bg-white rounded-2xl bg-opacity-90 justify-center items-between overflow-y-auto">
      <BiXCircle
        className="absolute text-primary text-4xl right-3 top-3 opacity-70 hover:opacity-100 cursor-pointer lg:text-gray-400 z-10"
        onClick={() => dispatch(setModalClose())}
      />
      <div className="relative min-h-[350px] w-full lg:relative lg:w-auto">
        <Image
          src={image}
          placeholder="blur"
          blurDataURL={image}
